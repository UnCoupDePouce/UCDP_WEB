...
function MethodBadge({method}: { method: string | null }) {
    ...
    return (
        <span
            className={`inline-flex px-2 py-0.5 rounded text-xs font-semibold ${METHOD_COLORS[m] ?? 'bg-gray-100 text-gray-600'}`}>
      {m}
    </span>
    );
}
...
export default function Logs() {
    ...
    useEffect(() => {
        fetchLogs({limit: '200', sort: 'desc'})
            .then(res => {
                console.log("Objet de réponse reçu dans le composant :", res);
                console.log("Tableau res.data spécifiquement :", res.data);
                setLogs(res.data || [])
            })
    ...
    return (
        <div className="px-8 py-8 flex flex-col gap-6">
            ...
            <div className="grid grid-cols-2 xl:grid-cols-4 gap-6">
                ...
                <StatCard title="Méthodes distinctes" value={stats.methodData.length} icon={Activity}
                          color="bg-teal-500"/>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
                {/* Timeline */}
                <div
                    className="xl:col-span-8 bg-white rounded-[20px] p-6 shadow-sm border border-gray-100 flex flex-col h-[280px]">
                    ...
                    <div className="flex-1">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={stats.timelineData} margin={{top: 10, right: 10, left: -20, bottom: 0}}>
                                ...
                                <XAxis dataKey="date" tick={{fill: '#718096', fontSize: 11}} axisLine={false}
                                       tickLine={false}/>
                                <YAxis tick={{fill: '#718096', fontSize: 11}} axisLine={false} tickLine={false}
                                       allowDecimals={false}/>
                                <Tooltip contentStyle={TOOLTIP_STYLE}/>
                                <Area type="monotone" dataKey="count" name="Erreurs" stroke="#ef4444" fillOpacity={1}
                                      fill="url(#colorErrors)" strokeWidth={2}/>
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                ...
                {/* Method breakdown */}
                <div
                    className="xl:col-span-4 bg-white rounded-[20px] p-6 shadow-sm border border-gray-100 flex flex-col h-[280px]">
                    ...
                    <div className="flex-1">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={stats.methodData} margin={{top: 0, right: 10, left: -20, bottom: 0}}>
                                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0"/>
                                <XAxis dataKey="method" tick={{fill: '#718096', fontSize: 11}} axisLine={false}
                                       tickLine={false}/>
                                <YAxis tick={{fill: '#718096', fontSize: 11}} axisLine={false} tickLine={false}
                                       allowDecimals={false}/>
                                <Tooltip contentStyle={TOOLTIP_STYLE}/>
                    ...
                    <div className="flex-1">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={stats.topRoutes} layout="vertical"
                                ...
                                <XAxis type="number" tick={{fill: '#718096', fontSize: 11}} axisLine={false}
                                       tickLine={false} allowDecimals={false}/>
                                <YAxis type="category" dataKey="route" tick={{fill: '#718096', fontSize: 11}}
            ...
            <div className="bg-white rounded-[20px] shadow-sm border border-gray-100 overflow-hidden">
                {/* Filters */}
                <div
                    className="p-6 border-b border-gray-100 flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                            ...
                            {paginated.map(log => {
                                ...
                                return (
                                    <>
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-3 text-gray-600 font-mono text-xs max-w-[200px] truncate"
                                                title={log.route ?? undefined}>
                                                {log.route ?? '—'}
                                            </td>
                                            <td className="px-6 py-3 text-gray-700 max-w-sm truncate"
                                                title={log.message}>
                                                {log.message}
                                            </td>
                                        </tr>
                                ...
                                <span>
                                  ...
                                  <span className="font-mono">{log.id}</span>
                                                        ...
                                                        {log.stack && (
                                                            <div>
                                                                <p className="text-xs font-semibold text-gray-700 mb-1">Stack
                                                                    trace</p>
                                                                <pre
                                                                    className="text-xs text-red-700 bg-red-50 border border-red-100 rounded-lg p-4 overflow-x-auto whitespace-pre-wrap break-all leading-5">
                                    {log.stack}
                                  </pre>
                                                            </div>
                                                        )}
                ...
                {totalPages > 1 && (
                    <div
                        className="px-6 py-4 border-t border-gray-100 flex items-center justify-between text-sm text-gray-500">
